...
(function (factory) {
  ...
  $.fn.tokenizer = function () {
    ...
    function _uniqueArray(array) {
      var newArray = array.filter(function (elem, index) {
        return array.indexOf(elem) === index;
      });
      return newArray;
    }
    function _getWrapperValues($input) {
      var $wrapperValues = $('<div></div>').addClass('tokenizer-values').width($input.width() - 7);
    ...
    function _getToken(value, settings) {
      var $token = $('<span></span>').addClass('tokenizer-token').text(value).attr('data-value', value).prepend($('<span></span>').addClass('tokenizer-token-close').text('x'));
    ...
    function _buildRegExp(separators) {
      var strReg = "[" + separators.map(function (sep) {
        return sep;
      }).join('') + "]";
    ...
    var _operations = {
      get: function get(args) {
        if (args.length > 1) throw new Error('Número de argumentos inválidos, se requiere (\'option\') solamente');

        return _getValues($this);
      },
      set: function set(args) {
        var str;
        if (args.length != 2) throw new Error('Número de argumentos inválidos, se requiere (\'option\', String|Array)');
      ...
      del: function del(args) {
        if (args.length != 2) throw new Error('Número de argumentos inválidos, se requiere (\'option\', String)');

        if (typeof args[1] === 'string') _removeToken($(".tokenizer-token[data-value='" + args[1] + "']"));else throw new Error('Segundo parámetro inválido, se requiere (\'option\', String)');
        return $this;
      }
    };
    ...
    if (!arguments.length || $.isPlainObject(arguments[0])) {
      ...
      $(document).on('keydown', function (e) {
        if (e.keyCode == $.tokenizer.KEY_CODE.SUPR) {
          var $elems = $('.tokenizer-token-active');
      ...
      return $this.each(function () {
        var $input = $(this);
        var $wrapper = $('<div></div>').addClass('tokenizer-wrapper').width($input.width());
        $input.wrap($wrapper);
        ...
        $input.off().on('keydown', function (e) {
          if (e.keyCode == settings.keyCodeCreate) {
            ...
            if ($input.val().trim()) {
              ...
              if (settings.max_input) tokens = tokens.splice(0, settings.max_input);
      ...
      switch (arguments[0]) {
        ...
        default:
          throw new Error("Opci\xF3n '" + arguments[0] + "' no v\xE1lida");
      }
    }
  };
  ...
  $.fn.tokenizer.defaults = {
    ...
    onDeleteToken: function onDeleteToken() {}
  };
});